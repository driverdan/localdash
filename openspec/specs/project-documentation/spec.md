...
The canonical project documentation — `README.md` (for humans) and `AGENTS.md`
(for coding agents) — must accurately describe the app that actually ships: its
user-facing features and the routes they serve, every built-in geo source, and the
public API surface. These requirements make documentation accuracy a stated
obligation so that when features, sources, or routes change, the docs are updated in
the same change rather than drifting behind the code.
