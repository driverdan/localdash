...
- [ ] 1.1 Add `fmtEventDate(startsAt: string, endsAt: string | null): string` to
...
- [ ] 2.1 Replace the `when` derivation in
...
- [ ] 3.1 Run `npm run check` in `frontend/` (svelte-check passes)
- [ ] 3.2 Rebuild with `sg docker -c 'docker compose up --build -d'` and visually confirm cards
