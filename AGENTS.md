...
For **GitHub PRs** (title or body), add the same `Co-Authored-By:` trailer in
the PR description, and state the tool and model in a short line, e.g.:

> Generated by **pi** running **claude-sonnet-4.5**.
>
> Co-Authored-By: pi (claude-sonnet-4.5) <noreply@agent.local>
