...
- [x] 1.1 In `frontend/src/lib/format.ts`, extract the local-midnight day-diff `fmtEventDate`
...
- [x] 2.1 In `frontend/src/features/home/api.ts`, `loadEvents()`: after fetching, filter the
...
- [x] 3.1 In `frontend/src/features/home/components/HomePage.svelte`, change the widget heading
      from "Current events" to "Today's events".
- [x] 3.2 In the same file, change the empty-state notice from "No current events." to
      "No events today."

## 4. Verify

- [x] 4.1 Rebuild the frontend (`docker compose up --build`) and confirm the home page shows a
      "Today's events" widget listing only events that start today, and the empty state when none
      do.
- [x] 4.2 Run the frontend checks (svelte-check / lint) to confirm no type or lint regressions.
