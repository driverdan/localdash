...
- [ ] 1.1 In `app/news/registry.py`, add the `chattgov` source to `SOURCES` ("City of Chattanooga",
...
- [ ] 1.2 Add a `feed_kind(url: str) -> str` helper that returns the feed's `kind` from `SOURCES`
  (default `"rss"`), mirroring the existing `uses_feed_tags(slug)` lookup.
- [ ] 1.3 Confirm `sync_registry` needs no change (it upserts URL/category/position only; `kind`
  lives in the registry, not the DB) — no `news_feeds` schema change, no Alembic migration.

## 2. Fetcher: HTML scrape path

- [ ] 2.1 Add `parse_html_listing(html: str, base_url: str) -> list[dict]` to `app/news/fetcher.py`:
...
- [ ] 2.2 Add `_fetch_html(url: str) -> str` using `httpx.AsyncClient(follow_redirects=True)` with
  the registry `USER_AGENT`; raise for non-success status so the per-feed `try/except` records it.
- [ ] 2.3 Branch `fetch_feed` on `feed_kind(feed.url)`: `html` → `_fetch_html` + `parse_html_listing`,
...
- [ ] 3.1 Save a representative `stay-informed/latest-news` HTML fixture (a few `.views-row`s,
  including one missing a title/link and one with an unparseable/missing date) under `tests/`.
- [ ] 3.2 Add offline unit tests for `parse_html_listing` (no network, no DB), matching
...
- [ ] 3.3 Add a test for `feed_kind`: `chattgov`'s feed → `"html"`, an existing RSS feed → `"rss"`,
  an unknown URL → `"rss"` default.

## 4. Verify end-to-end

- [ ] 4.1 Run the full test suite and `ruff`/formatting; ensure no regressions in the RSS path.
- [ ] 4.2 Rebuild and run via Docker (`docker compose up --build`), trigger `POST /api/v1/news/refresh`,
