...
- [ ] 1.1 In `frontend/src/features/home/api.ts`, add types for the outage digest (per-service `{count, customers}` summary) and a `loadOutages()` loader that concurrently fetches `/api/v1/timeseries/entities?source=epb` and `/api/v1/timeseries/sources`, reduces active features to the per-service summary (missing/non-positive `customer_quantity` counts as zero), and derives `epb` enablement (sources fetch failure → fall back to rendering from entities alone).
- [ ] 1.2 In `frontend/src/features/home/state.svelte.ts`, add outage digest state: the summary, an enabled/hidden flag, and `outagesLoaded`/`outagesError` flags matching the sibling widgets.

## 2. Widget component

- [ ] 2.1 Create `frontend/src/features/home/components/OutageDigest.svelte`: heading "Outages" with a client-side "View all →" link to `/map`; one row per service with active outages ("N power outage(s)" / "N fiber outage(s)", "· X customers" only when the summed quantity is positive); "No current outages" zero state; error notice on failed load; render nothing at all when the `epb` source is disabled.
...
- [ ] 4.3 Verify the hidden state: with the `epb` source disabled, the widget renders nothing while the other widgets are unaffected.
