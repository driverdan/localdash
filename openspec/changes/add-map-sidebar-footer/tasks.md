...
- [x] 1.1 In `frontend/src/features/timeseries/components/Dashboard.svelte`, import `SiteFooter`
      from `../../../lib/SiteFooter.svelte` (matching the import style used by `HomePage`,
      `NewsFeed`, and `EventsPage`)
- [x] 1.2 Render `<SiteFooter />` as the last child of `<aside id="sidebar">`, immediately after
      `<IncidentTable />` — no wrapper element, no class, no inline style
- [x] 1.3 Confirm no changes are made to `frontend/src/styles/timeseries.css` (`#sidebar`) or
      `frontend/src/styles/base.css` (`.site-footer`)

## 2. Keep documentation accurate

- [x] 2.1 Update the leading comment in `frontend/src/lib/SiteFooter.svelte` so it no longer says
