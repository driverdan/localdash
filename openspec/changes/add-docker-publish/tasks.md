...
- [ ] 1.1 Add a `docker` job to `.github/workflows/tests.yml` with `needs: [pytest, frontend]`,
      running on `ubuntu-latest`
- [ ] 1.2 Set up buildx (`docker/setup-buildx-action`) and log in to Docker Hub
...
- [ ] 1.3 Generate tags with `docker/metadata-action` for image `driverdan/localdash`: `latest`
      on the default branch plus a short-SHA tag (`type=sha`)
- [ ] 1.4 Build with `docker/build-push-action`: `platforms: linux/amd64`,
...
- [ ] 2.1 Validate the workflow YAML locally (e.g. `actionlint` or a YAML parse) before opening
      the PR
