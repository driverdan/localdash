...
- [x] 1.1 In `app/api/events.py`, change the `upcoming` predicate from
...
- [x] 2.1 In `tests/test_api_events.py`, extend the default-listing test (or add a sibling) to
...
- [x] 3.2 Rebuild with `docker compose up --build` (via `sg docker`) and confirm
