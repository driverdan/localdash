...
builds the production image with buildx on any CI run whose diff could affect the image — so a
Dockerfile break can never merge — and pushes tagged images to the `driverdan/localdash` Docker Hub
repository only from pushes to `main` that passed the test jobs. Authentication uses the
`DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN` repository secrets (a Docker Hub access token, not the
account password); pull-request runs never touch credentials.
