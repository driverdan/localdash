...
- [ ] 1.1 Add `airnow_api_key: str = ""` to `Settings` in `app/config.py` (env `AIRNOW_API_KEY`;
      empty disables the AirNow fetch), documented alongside the weather settings
- [ ] 1.2 Create `app/weather/airnow.py` with `parse_airnow(payload) -> dict | None`: pick the
...
- [ ] 1.3 Add an AirNow current-observations fixture (O3 + PM2.5 entries with differing AQI) and
      offline tests for `parse_airnow`: max-across-pollutants, -999 sentinel skipped, empty
      array → `None`

## 2. Backend: service integration

- [ ] 2.1 Add `_fetch_aqi()` to `WeatherService` (`app/weather/service.py`): return `None`
      ...
      failure
- [ ] 2.2 Join `_fetch_aqi` to the `asyncio.gather` in `_refresh` and add `aqi` to the returned
      payload; keep the both-NWS-failed error check ignoring AQI (AQI success never rescues,
      AQI failure never breaks)
- [ ] 2.3 Service tests with `httpx.MockTransport`: payload carries `aqi` when AirNow responds;
...
- [ ] 3.1 Extend the `Weather` types in `frontend/src/features/home/api.ts` with
      `aqi: WeatherAqi | null` (`value`, `category`, `category_name`, `pollutant`)
- [ ] 3.2 Render the AQI chip in `WeatherStrip.svelte`: compact pill reading
...
- [ ] 4.1 Run backend tests (`pytest`) and frontend checks (`svelte-check`/build) clean
- [ ] 4.2 Rebuild via `sg docker -c 'docker compose up --build -d'` and confirm
