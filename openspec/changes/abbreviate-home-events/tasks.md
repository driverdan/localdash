...
- [x] 1.1 Create `frontend/src/features/home/components/EventDigest.svelte`: takes an
...
- [x] 1.2 In `HomePage.svelte`, render `EventDigest` instead of `EventCard` in the events
  widget, drop the `id="events"` on the widget body, and remove the now-unused
  `EventCard` import from `features/events`

## 2. Styling

- [x] 2.1 Add `.event-digest` row styles to `frontend/src/styles/home.css` (theme variables,
  ...
  `#news` halves)
- [x] 2.2 Update the `home.css` header comment: card reuse applies where the digest is the
  full card (news); home owns compact renderings otherwise (weather, events)

## 3. Events public surface cleanup

- [x] 3.1 Verify nothing imports `EventCard` from `features/events/index.ts` anymore; drop
  the export and its home-digest comment (events' own page imports the component
  directly)

## 4. Verification

- [x] 4.1 Run the frontend checks (svelte-check / lint / format) and fix any fallout
- [x] 4.2 Rebuild and run via `sg docker -c 'docker compose up --build'` and verify on `/`:
