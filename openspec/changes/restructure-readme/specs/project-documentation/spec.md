...
**Reason**: Requiring a hand-maintained API reference in the README is what
produced the stale endpoint tables — they were never updated for the Weather
feature and still named a `/api/v1/timeseries/ws` WebSocket that was removed in
favour of the global bus at `/api/v1/ws`. The app already serves interactive,
always-current OpenAPI documentation.

**Migration**: Replaced by the ADDED requirement "README points at the served API
reference". API conventions for contributors remain specified in `AGENTS.md`.
