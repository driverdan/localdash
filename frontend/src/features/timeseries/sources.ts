...
export const SOURCES: Record<string, SourceConfig> = {
  ...
  epb: {
    ...
    title: (p) => str(p.label) || (p.service === "fiber" ? "Fiber Outage" : "Energy Outage"),
    location: () => "",
    jurisdiction: (p) => cap(str(p.service)),
    detail: (p, d) => [
      ["Status", catLabel(str(p.status))], ["Service", cap(str(p.service))],
...
// ---- display helpers ---------------------------------------------------------
export const isClosed = (f: TrackedFeature): boolean => f.properties.active === false;
export const catLabel = (cat: string): string => cap(String(cat || "").replace(/_/g, " "));
