...
- [ ] 1.1 Create `frontend/src/lib/prefs.ts` with `loadPrefs(key)` (parse JSON from localStorage;
...
- [ ] 2.1 In `timeseries/state.svelte.ts`, initialize `selectedSources` and `categories` from
...
- [ ] 2.2 Register a module-scope `$effect.root` that saves `{selectedSources, categories,
...
- [ ] 2.3 Add a `resetFilters()` action that removes `localdash.map`, restores all-on defaults and
      `showClosed=false`/`closedWindow=60`, and suppresses the save effect for that change so the
      key stays absent afterward
- [ ] 2.4 Add a "Reset filters" button to `FilterPanel.svelte` wired to `resetFilters()`, styled
      consistently with the existing panel controls

## 3. Events preferences

- [ ] 3.1 In `events/state.svelte.ts`, initialize `topics` and `maxMiles` from `localdash.events`
...
- [ ] 4.1 In `news/state.svelte.ts`, initialize `activeTab`, `hours`, and `multiOnly` from
...
- [ ] 4.2 Make the feed render the "All" view when the saved `activeTab` is not among the available
      tabs after stories load

## 5. Verification

- [ ] 5.1 Run `npm run check` in `frontend/` and rebuild via
      `sg docker -c 'docker compose up --build -d'`
- [ ] 5.2 Verify in the browser: toggle map sources/categories/show-closed, events topics/distance,
      news tab/window/multi-only; reload after each and confirm restoration; confirm search boxes
      reset on reload
- [ ] 5.3 Verify allowlist + reset semantics: with saved prefs, `localdash.map` lists only checked
